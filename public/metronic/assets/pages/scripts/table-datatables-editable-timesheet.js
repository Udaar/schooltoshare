var TableDatatablesEditable = function () {
    ...
    var handleTable = function () {
        ...
        function editRow(oTable, nRow) {
            //  $th = $(nRow).closest('table').find('th').eq($(nRow).index());
            // console.log("ssss",$th.attr('key'));
            var aData = oTable.fnGetData(nRow);
            var jqTds = $('>td', nRow);
            for(var i=0;i<jqTds.length-2;i++){
                $th = $(nRow).closest('table').find('th').eq(i).attr('key');
                jqTds[i].innerHTML = '<input name = "'+$th+'"id="input'+i+'" type="text" class="form-control input-small" value="' + aData[i] + '">';
            }
        ...
        var table = $('#sample_editable_1_custom');
        ...
        $('#sample_editable_1_new_custom').click(function (e) {
            e.preventDefault();
