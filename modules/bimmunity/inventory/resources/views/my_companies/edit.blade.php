@extends('layouts.app')

@section('content')

<section class="my-company-edit-section ifm-form">
    <div class="portlet light ifm-border-light-grey-all">
        <div class="portlet-title ifm-border-light-grey-bottom">
            <div class="caption">
                <h3 class="ifm-grey ifm-no-margin-all inline-block capitalize normal title">
                    <img src="data:image/png;base64,">
                    Edit Your Company
                </h3>
            </div>
        </div>
        <div>
            @include('metronic-templates::common.errors')
        </div>
        <div class="portlet-body form">
            <div class="row">
            {!! Form::model($myCompany, ['route' => ['myCompanies.update', $myCompany->id], 'method' => 'patch']) !!}

                @include('inventory::my_companies.fields')

            {!! Form::close() !!}
            </div>
        </div>
    </div>
</section>
@endsection

@push('scripts')
<script>
    $(document).on('keyup change', '#debit , #credit', function() {
        console.log('dsdasdas');
      $('#total').val($('#debit').val()-$('#credit').val());
    });
</script>
@endpush