        ...
        <section class="ifm-white-bg order-show-section clearfix info-section ifm-padding-all ifm-padding-md-all ifm-border-light-grey-all">
            ...
            <div class="ifm-padding-md-bottom ifm-margin-md-bottom ifm-border-light-grey-bottom clearfix">
                <h3 class="ifm-grey ifm-no-margin-all inline-block uppercase title">
                    ...
                    Order Number #{!! $inOrder->order_number !!}
                </h3>
            </div>
            {{-- Errors Div --}}
            <div>
            ...
            <div class="container-fluid">
                @include('inventory::in_orders.show_fields')
            </div>
           
            <div class="col-xs-12">
                @if(($inOrder['element_status'] >0 && $inOrder['status'] == 2) || ($inOrder['element_status'] < 1 ))
                <form id="approve_form" action="{!! route('inOrders.sales_action',3) !!}" method="post" class="col-xs-2" >
                    @if(isset($ticket_id))
                        <input type="hidden" name="ticket_id" value="{{$ticket_id}}">
                        <input type="hidden" name="action_id" value="{{$action_id}}">
                    @endif
                    <input type="hidden" name="approve_notes" id="approve_notes">
                    <input type="hidden" name="order_id" value="{{ $inOrder['id'] }}">
                ...
                <form id="pending_form" action="{!! route('inOrders.sales_action',1) !!}" method="post" class="col-xs-2">
                    @if(isset($ticket_id))
                        <input type="hidden" name="ticket_id" value="{{$ticket_id}}">
                        <input type="hidden" name="action_id" value="{{$action_id}}">
                    @endif
                ...
                <form id="refuse_form" action="{!! route('inOrders.sales_action',2) !!}" method="post" class="col-xs-2">
                    @if(isset($ticket_id))
                        <input type="hidden" name="ticket_id" value="{{$ticket_id}}">
                        <input type="hidden" name="action_id" value="{{$action_id}}">
                    @endif
