...
<section class="purchasing-section ifm-padding-md-all ifm-white-bg ifm-border-light-grey-all">
    ...
    <div class="row">
        ...
        <div class="ifm-padding-sm-all">
            @include('inventory::finance.sales_approve_finance_table')
        </div>
    </div>
</section>
@endsection
    ...
    <script>
                ...
                "aoColumnDefs": [
                    {
                        ...
                        "mRender": function ( data, type, full ) {
                            var url = '<a href="{{ route("finance.approved_out_order_show",":id")}}" class="ifm-grey" style="text-decoration:underline">'+ data + '</a>';
        ...
        function approved_orders(element){
            ...
            if($('#'+element_id).is(':checked')==true){
                $.get('{{ url('finance') }}/ajax_get_approved_out_orders/data/all', function(data) {
        ...
        function unapproved_orders(element){
            var element_id = element.id;
            if($('#'+element_id).is(':checked')==true){
                $.get('{{ url('finance') }}/ajax_get_unapproved_out_orders/data/all', function(data) {
                    var table = $('#ajaxTable').DataTable();
                    $.each(data, function(index,subCatObj) {
                        var temp_status ;
