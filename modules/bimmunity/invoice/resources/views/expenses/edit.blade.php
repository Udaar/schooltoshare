@extends('layouts.app')

@section('content')

<section class="expenses-edit-section ifm-form">
    <div class="portlet light ifm-border-light-grey-all">
        <div class="portlet-title ifm-border-light-grey-bottom">
            <div class="caption">
                <h3 class="ifm-grey ifm-no-margin-all inline-block capitalize normal title">
                    <img src="data:image/png;base64,">
                    edit expenses
                </h3>
            </div>
        </div>
        <div>
            @include('metronic-templates::common.errors')
        </div>
        <div class="portlet-body">
            <div class="row">
			{!! Form::model($expenses, ['files'=>'true','route' => ['expenses.update', $expenses->id], 'method' => 'patch']) !!}
                @include('bimmunity/invoice::expenses.fields')
			{!! Form::close() !!}
            </div>
        </div>
    </div>
</section>

@endsection
