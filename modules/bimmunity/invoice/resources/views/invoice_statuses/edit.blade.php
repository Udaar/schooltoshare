@extends('layouts.app')

@section('content')

<section class="invoice-statuses-edit-section ifm-form">
    <div class="portlet light ifm-border-light-grey-all">
        <div class="portlet-title ifm-border-light-grey-bottom">
            <div class="caption">
                <h3 class="ifm-grey ifm-no-margin-all inline-block capitalize normal title">
                    <img src="data:image/png;base64,">
                    edit invoice statuses
                </h3>
            </div>
        </div>
        <div>
            @include('metronic-templates::common.errors')
        </div>
        <div class="portlet-body">
            <div class="row">
			{!! Form::model($invoiceStatus, ['route' => ['invoice_statuses.update', $invoiceStatus->id], 'method' => 'patch']) !!}
                @include('bimmunity/invoice::invoice_statuses.fields')
			{!! Form::close() !!}
            </div>
        </div>
    </div>
</section>

@endsection
